...
% 2024.1.25
function[strmeanpof] = repevo2cr_pofvrf(str,Lnum,Cnum,Dnum,N,repItnum,kmin,kmax,Gamestc,lambda,q,err,alpha,beta)
...
for it = 1:repItnum
    ...
    if it > repItnum/4 % after one forth of iterations
        ...
        Defpof = R*c*sum(acts)/k;% defect payoff
        Cooppof = Defpof - c; % cooperate payoff   
        agtpof = acts*Cooppof + (~acts)*Defpof; % payoff in this round
