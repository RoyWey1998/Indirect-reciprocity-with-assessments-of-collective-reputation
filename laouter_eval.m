...
function[exre] = laouter_eval(agtlist,RM,lambda_list,acts,restagt,nAgtout,str,k,q,err)
kout = sum(nAgtout); % number of players outside
exgre1 = zeros(nAgtout(1),k); exgre2 = zeros(nAgtout(2),k);exgre3 = zeros(nAgtout(3),k);exre = zeros(kout,k); % init
...
for j = 1:k
    ...
    exgre3(resum(nAgtout(1)+nAgtout(2)+1:kout)>=lambda_list(3)*(k-1),j) = 1; % decide the group reputation of recipients
end
exgre = [exgre1;exgre2;exgre3];
% update reputation image for each player outside
for j = 1:kout
